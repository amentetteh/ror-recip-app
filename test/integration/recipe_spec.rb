...
RSpec.describe 'Recipes Page', type: :system do
  describe 'Recipe /index' do
    before(:each) do
      ...
      @recipe1 = Recipe.create(name: 'Pizza', preparation_time: 1, cooking_time: 2, description: 'Melt and cook', public: true, user: @user)
      @recipe2 = Recipe.create(name: 'Cake', preparation_time: 1, cooking_time: 2, description: 'Melt, boil and cook', public: true, user: @user)
      @user.skip_confirmation!
      @user.save!
      visit new_user_session_path
      fill_in 'user_email', with: 'tkamen@outlook.com'
      fill_in 'user_password', with: '1233456'
      click_button 'Log in'
      sleep(1)
      visit recipes_path
    end
  ...
  describe 'Recipe /new' do
    before(:each) do
      ...
      @recipe = Recipe.create!(name: 'Wasawasa', preparation_time: 1, cooking_time: 2, description: 'description',
    ...
    it 'creates a new recipe' do
      fill_in 'recipe_name', with: 'Dzongoli'
      fill_in 'recipe_preparation_time', with: 1
      fill_in 'recipe_cooking_time', with: 2
      fill_in 'recipe_description', with: 'description'
      click_button 'Create recipe'
